...
while (my $line = <EXECLOG>) {
	if ($line =~ /"vAppVersion = (v\d+\.\d+\.\d+\.\d+)"/) {
		$version = $1;
	}
	elsif ($line =~ /[\\\/]+([^\\\/]+?)[\\\/]+default.cfg/) {
		$scriptname = $1;
	}
...
if (!$version) {
	$version = $timestamp;
	print "version is not found, using time stamp($timestamp) instead\n";
	$hang = 1;
}
if ($scriptname eq 'tt') {
...
my $status = copy("output.txt", "../$version.$scriptname.output.$starttime.$time.txt");
if (!$status) {
	print "copy(\"output.txt\", \"../$version.$scriptname.output.$starttime.$time.txt\");\n";
