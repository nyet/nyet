...
foreach my $type (sort keys %skim) {
	...
	foreach my $metric (@{$skim{$type}{metrics}}) {
		...
		my $cmdLine = qq{java -Xmx128M -Duser=Guest -Dpassword=Guest -Dhost=xxx -Dport=5001 -jar "../CLWorkstation.jar" get historical data from agents matching \"$agents\" and metrics matching \"$metric\" $time > $rawdata_file};
		print {$LOG} "		$cmdLine\n";
